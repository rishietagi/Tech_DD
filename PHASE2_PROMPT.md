...
1. `DD_master.md` — the domain authority. A technology due diligence reference built
...
2. `PHASE2_SCOPE_ENGINE.md` — the build spec for this phase: architecture, schemas,
...
4. `initial_plan.md` — what Phase 1 built, so you know what already exists.

Precedence on conflict: `CLAUDE.md` > `DD_master.md` > `PHASE2_SCOPE_ENGINE.md` >
`initial_plan.md`. If you find a real conflict, tell me rather than picking silently.
...
Then build it, following `PHASE2_SCOPE_ENGINE.md` §12 in order, committing after each
numbered step.
...
3. **Transcribe `DD_master.md` §6 faithfully.** The sourced question sets in there are
   ...
   `source_provenance: sourced` or `extended` — `DD_master.md` §16 explains why that
   distinction has to survive into the output.

4. **Implement the whole rule set** in `DD_master.md` §15 — A1–A11, M1–M7, D1–D10,
...
7. **Add the intake fields first** (`PHASE2_SCOPE_ENGINE.md` §9.3): `deal_type`,
...
8. **Golden-case tests are the deliverable, not an afterthought.** Build the six cases
   in `PHASE2_SCOPE_ENGINE.md` §10 as intake fixtures with snapshotted deterministic
...
When you finish, verify against the acceptance criteria in `PHASE2_SCOPE_ENGINE.md`
